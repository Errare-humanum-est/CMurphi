/**CFile***********************************************************************
  ...
  Synopsis    [File for declarations]
...
:  typedecl(), accuracy(0), exponent(0), exponent_value(0)
{
  ...
  Error.CondError((ex->getvalue() < 10) || (ex->getvalue() > DBL_MAX_10_EXP),	//IM: fixed, now we have directly the value
		  "Exponent max value must be between 10 and %d.",
		  DBL_MAX_10_EXP);
